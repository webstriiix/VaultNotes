...
const Notes = () => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await actor.delete_note(deleteNoteId);
  ...
  const handleSearchResultSelect = (result) => {
    ...
    const existingNote = notes.find(
      (note) => note.id === Number(result.noteId)
    );
    if (existingNote) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredNotes.map((note) => (
              <Card
                ...
                onPress={() => {
                  console.log(
                    `Card clicked, navigating to: /update-note/${note.id}`
                  );
                    ...
                    <Dropdown>
                      ...
                      >
                        <DropdownItem
                          ...
                          onPress={() => {
                            console.log(
                              `Dropdown Edit clicked for note: ${note.id}`
                            );
                            navigate(`/update-note/${note.id}`);
                          }}
                        >
